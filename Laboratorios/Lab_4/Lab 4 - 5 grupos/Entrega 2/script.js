...
function crearCitaMedica() {
    var ajax = new XMLHttpRequest();
    var datos = new FormData(document.querySelector('#formCitaMedica'));
    ajax.open("POST", "createCitaMedica.php", true);
    ajax.onreadystatechange = function () {
        if (ajax.readyState == 4 && ajax.status == 200) {
                document.querySelector('#contenido').innerHTML = ajax.responseText;
                setTimeout(function(){
                    cargarContenido('listarCitasMedicas.php');
                }, 3000);
        }
    };
    ajax.send(datos);
...
function editarEstadoCita() {
    var ajax = new XMLHttpRequest();
    var datos = new FormData(document.querySelector('#formEditEstadoCita'));
    ajax.open("POST", "editCitaEstado.php", true);
...
function filtrarCitas() {
    var m = document.getElementById('filtroMedico').value;
    var p = document.getElementById('filtroPaciente').value;
    var e = document.getElementById('filtroEstado').value;

    var ajax = new XMLHttpRequest();
    ajax.open("GET", "listarCitasMedicas.php?m=" + m + "&p=" + p + "&e=" + e, true);
    ajax.onreadystatechange = function() {
        if (ajax.readyState == 4 && ajax.status == 200) {
            document.querySelector('#contenido').innerHTML = ajax.responseText;
        }
